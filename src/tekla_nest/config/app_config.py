...
class AppConfig:
    ...
    materials: list[str] = field(
        default_factory=lambda: ["S235JR", "S275JR", "S355JR"]
    )
...
def _read_yaml(config_path: Path) -> dict:
    ...
    try:
        ...
        raise ConfigError(
            ...
            "Run:  uv pip install PyYAML   (or:  pip install PyYAML)"
        )
...
def _apply_section(
    cfg: AppConfig, raw: dict, section: str, mappings: dict, base_dir: Path
) -> None:
    """Apply a config section's values to cfg with type checking."""
    data = raw.get(section, {})
    if not isinstance(data, dict):
        raise ConfigError(
            f"config.yaml section '{section}' must be a mapping, "
    ...
    for yaml_key, (attr, converter) in mappings.items():
        if yaml_key in data:
            ...
            try:
                ...
                raise ConfigError(
                    f"config.yaml [{section}] key '{yaml_key}': "
                    f"invalid value {val!r} — {exc}"
                )
...
def load_config(path: str | None = None) -> AppConfig:
    ...
    # ── nesting section ──
    _apply_section(cfg, raw, "nesting", {
    ...
    # ── licensing section ──
    _apply_section(cfg, raw, "licensing", {
