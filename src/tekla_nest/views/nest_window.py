...
from ..design_system import set_accessibility, set_action_enabled
...
class NestWindow(QMainWindow):
    ...
    def __init__(
        ...
        for command_id in (
            "load_tekla", "load_parts_csv", "clear_parts",
            "auto_stock", "calculate", "export_pdf",
        ):
    ...
    def closeEvent(self, event: QCloseEvent) -> None:  # noqa: N802
        ...
        pdf_btn = box.addButton(
            tr("dialogs.buttons.export_pdf"), QMessageBox.ButtonRole.ActionRole
        )
    ...
    def _on_color_schema(self) -> None:
        ...
        dlg = ColorSchemaDialog(
            primary=cfg.primary_color, accent=cfg.accent_color,
            background=cfg.background, parent=self,
        )
    ...
    def _show_error(self, message: str) -> None:
        ...
        if message.startswith("modal:") or _is_blocking_error(message):
            QMessageBox.warning(
                self, tr("dialogs.messages.error_title"), localized,
            )
    ...
    def _show_csv_error(self, title: str, detail: str) -> None:
        ...
        copy_btn = box.addButton(
            tr("dialogs.buttons.copy"), QMessageBox.ButtonRole.ActionRole
        )
    ...
    def _on_operation_progress(self, operation: str, message: str) -> None:
        self._status.set_status(
            tr_error(message) or self._operation_label(operation), "loading"
        )
    ...
    def _on_operation_failed(self, operation: str, message: str) -> None:
        self._status.set_status(
            tr_error(message) or self._operation_label(operation), "error"
        )
        self._sync_action_state()
    ...
    def _sync_action_state(self) -> None:
        ...
        self._set_enabled(
            "clear_parts", has_parts and not busy, tr("disabled.no_parts_to_clear")
        )
        ...
        for command_id in ("export_pdf", "export_excel", "export_csv"):
            self._set_enabled(
                command_id, has_result and not busy, tr("disabled.run_before_export")
            )
