...
from ..design_system import set_accessibility
...
class ActivationDialog(QDialog):
    ...
    def __init__(self, manager: LicenseManager, parent=None) -> None:
        ...
        info = QLabel(
            tr("dialogs.activation.instructions")
        )
        ...
        mid_label = QLabel(
            tr("dialogs.activation.machine_id", machine_id=manager.machine_id[:16])
        )
    ...
    def _set_activation_busy(self, busy: bool) -> None:
        ...
        self._activate_btn.setText(
            tr("dialogs.activation.activating")
            if busy
            else tr("dialogs.activation.activate")
        )
