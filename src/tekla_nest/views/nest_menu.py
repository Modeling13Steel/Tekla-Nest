...
from ..design_system import CommandDescriptor, apply_action_descriptor
from ..design_system.tokens import Theme
from ..i18n import available_languages, current_language, tr
...
_FILE_IMAGE_CMDS: tuple[_Cmd, ...] = (
    _Cmd("load_image", "_on_load_image"),
)
...
_NEST_CMDS: tuple[_Cmd, ...] = (
    _Cmd("calculate", "_on_calculate", "Ctrl+R"),
)
_VIEW_CMDS: tuple[_Cmd, ...] = (
    _Cmd("color_schema", "_on_color_schema"),
)
...
class MenuBuilder:
    ...
    def _build_language_menu(
        self, view_menu: QMenu, menus: dict[str, QMenu]
    ) -> dict[str, QAction]:
    ...
    def _build_theme_menu(
        self, prefs_menu: QMenu, menus: dict[str, QMenu]
    ) -> dict[Theme, QAction]:
        ...
        for theme in _THEME_ORDER:
            ...
            if handler is not None:
                action.triggered.connect(
                    lambda _checked=False, t=theme, h=handler: h(t)
                )
            group.addAction(action)
