...
from tekla_nest.config.app_config import load_config
...
def main() -> None:
    ...
    print(f"  {'-'*4}  {'-'*8}  {'-'*8}  {'-'*12}  {'-'*10}")
    for p in parts:
        print(f"  {p.quantity:>4}  {p.length:>8.0f}  {p.reference:<8}  {p.profile:<12}  {p.material}")
    ...
    for profile in profiles:
        ...
        for i, bar in enumerate(result.bars, 1):
            ...
            print(f"    Bar {i}: {bar.original_length:.0f}mm"
                  f"  →  [{cuts_str}]"
    ...
    if all_profile_results:
        ...
        nest_result = NestResult(profiles=[
            ProfileResult(profile=profiles[i], bars=r.bars, waste_pct=r.waste_pct, scrap_pct=r.scrap_pct)
