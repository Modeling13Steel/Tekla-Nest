...
def export_csv(
    ...
    with out.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        for prof in result.profiles:
            profile_header = (
                f"{prof.profile} / {prof.material}"
                if prof.material else prof.profile
            )
            writer.writerow(
                [f"{profile_header} - {labels.waste}: {prof.waste_pct:.2f}%"] + pad(1)
            )
        ...
        if agg:
            ...
            for key in sorted_keys:
                ...
                writer.writerow([
                    profile, material, int(length), count, f"{linear_m:.2f}",
                ] + pad(5))
            ...
            writer.writerow([
                labels.total, "", "", "", f"{grand_total_m:.2f}",
            ] + pad(5))
