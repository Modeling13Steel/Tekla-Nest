...
class NestPresenter(QObject):
    ...
    def load_parts_from_csv(self, path: str) -> None:
        ...
        if not self._begin_operation(operation, source=path):
            if sys.stdout is not None:
                print("[TEKLANEST] load_parts_from_csv BLOCKED (operation already running)", flush=True)
            return
        try:
    ...
    def auto_populate_stock(self) -> None:
        ...
        if not self._begin_operation(operation):
            return
        pairs = sorted({
            (p.profile.strip(), (p.material or "").strip())
            for p in self._parts if p.profile
        })
        if not pairs:
            self.notice.emit(
                "Auto-stock skipped: load parts first to seed default stock."
            )
            self._complete_operation(operation)
            return
    ...
    def load_client_stock_csv(self, path: str) -> None:
        ...
        if not self._begin_operation(operation, source=path):
            if sys.stdout is not None:
                print("[TEKLANEST] load_client_stock_csv BLOCKED (operation already running)", flush=True)
            return
        try:
            self._client_stock = _sort_stock(load_stock_csv(path))
            if sys.stdout is not None:
                print(f"[TEKLANEST] load_client_stock_csv OK  entries={len(self._client_stock)}", flush=True)
    ...
    def run_optimization_async(self) -> None:
        ...
        worker.progress.connect(
            lambda message: self.operation_progress.emit(operation, message)
        )
    ...
    def request_stock(self, profile: str, length: float, piece_count: int) -> None:
        ...
        """
        msg = (
            f"Stock request: {piece_count} piece(s) of {profile} "
            f"at {length:.0f} mm need stock."
        )
        LOGGER.info(msg)
    ...
    def _compute_nest_result(
        ...
        for profile, material in groups:
            ...
            group_parts = [
                p for p in parts
                if p.profile == profile and (p.material or "") == material
            ]
    ...
    def _on_async_optimization_finished(
        ...
        self.optimization_summary_changed.emit(
            OptimizationSummary.from_nest_result(result)
        )
        self.state_changed.emit()
        self._complete_operation(operation)
