...
def _make_part(profile, material, reference="P1", length=3000.0, qty=1):
    ...
    return PartEntry(quantity=qty, length=length, reference=reference,
...
def _make_stock(profile, material, length, source="Mercado"):
    ...
    return StockEntry(quantity=1, length=length, priority=0,
                      profile=profile, material=material, source=source)
...
def test_parts_and_stock_same_profile_material_order():
    ...
    sorted_stock_keys = list(dict.fromkeys(  # deduplicate preserving order
...
def test_csv_stock_notice_emitted_after_complete_operation():
    ...
    with patch("tekla_nest.presenters.nest_presenter.load_stock_csv",
...
def test_csv_parts_empty_notice_emitted_after_complete_operation():
    ...
    with patch("tekla_nest.presenters.nest_presenter.load_parts_csv",
               return_value=[]):
