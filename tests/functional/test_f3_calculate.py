...
class TestAsyncCalculation:
    ...
    def test_async_double_start_is_blocked(
        self, journey, parts_csv_factory, collector, qtbot,
    ):
        journey.load_parts(parts_csv_factory())
        journey.auto_stock()
        with qtbot.waitSignal(
            journey.presenter.operation_completed, timeout=5000,
    ...
    def test_piece_equal_to_bar_fits_with_kerf(
        ...
        for profile in result.profiles:
            assert not profile.unfit_pieces, (
                f"Feedback #7 regression — kerf made bar-length piece unfit: "
                f"{profile.unfit_pieces}"
            )
