...
def test_auto_stock_covers_unconfigured_material(presenter, material):
    presenter.set_parts([
        PartEntry(quantity=2, length=3000, reference="P1",
                  profile="IPE200", material=material),
    ])
...
def test_auto_stock_covers_custom_profile_and_material(presenter):
    presenter.set_parts([
        PartEntry(quantity=2, length=3000, reference="P1",
                  profile="L60x60x6", material="AlMgSi"),
    ])
    presenter.auto_populate_stock()
    presenter.run_optimization()
...
def test_auto_stock_covers_over_length_pieces(presenter):
    ...
    """
    presenter.set_parts([
        PartEntry(quantity=1, length=18000, reference="P1",
                  profile="WI300-15-20*300", material="STEEL_UNDEFINED"),
    ])
    presenter.auto_populate_stock()
    presenter.run_optimization()
    result = presenter._last_result
    ...
    pr = result.profiles[0]
    assert pr.unfit_pieces == [], (
        f"18m piece left unfit: {pr.unfit_pieces}"
    )
...
def test_auto_stock_rounds_up_over_length_to_next_500(presenter):
    presenter.set_parts([
        PartEntry(quantity=1, length=13250, reference="P1",
                  profile="WI300-15-20*300", material="STEEL_UNDEFINED"),
    ])
    presenter.auto_populate_stock()
    bars_for = [s for s in presenter._market_stock
                if s.profile == "WI300-15-20*300"
                and s.material == "STEEL_UNDEFINED"]
