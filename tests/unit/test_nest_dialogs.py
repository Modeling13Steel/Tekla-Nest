...
def test_prompt_handles_cancel(qtbot, monkeypatch):
    monkeypatch.setattr(
        nest_dialogs.QFileDialog, "getOpenFileName", lambda *a, **k: ("", "")
    )
    monkeypatch.setattr(
        nest_dialogs.QFileDialog, "getSaveFileName", lambda *a, **k: ("", "")
    )
    monkeypatch.setattr(
        nest_dialogs.QInputDialog, "getText", lambda *a, **k: ("", True)
    )
    presenter = MagicMock()
    nest_dialogs.prompt_load_parts_csv(None, presenter)
    nest_dialogs.prompt_load_stock_csv(None, presenter)
