...
def _bar(length: float, source: str = "market",
         material: str = "S275JR") -> BarResult:
    return BarResult(
...
def test_aggregate_groups_by_source():
    profile = ProfileResult(
        ...
        bars=[_bar(6000, source="market"),
              _bar(6000, source="client")],
    )
...
def test_aggregate_groups_by_profile_and_material():
    p1 = ProfileResult(
        profile="HEA240", material="S275JR", bars=[_bar(6000)],
    )
    p2 = ProfileResult(
        profile="HEA240", material="S355JR", bars=[_bar(6000)],
    )
    p3 = ProfileResult(
        profile="IPE200", material="S275JR", bars=[_bar(6000)],
    )
