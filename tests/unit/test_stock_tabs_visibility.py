...
from tekla_nest.config.app_config import get_config, load_config, reset_config
...
def test_auto_market_stock_emits_one_row_per_material():
    ...
    for length in lengths:
        mats_for_length = {
            s.material for s in stock
            if s.profile == "HEA240" and s.length == length
        }
        assert mats_for_length == set(cfg.materials), (
            f"length {length} mm missing materials: "
            f"{set(cfg.materials) - mats_for_length}"
        )
...
def test_market_stock_tab_renders_material_rows(qtbot):
    ...
    materials_in_view = {
        model.data(model.index(r, material_col))
