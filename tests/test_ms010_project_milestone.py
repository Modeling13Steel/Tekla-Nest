...
def test_subprocess_parse_dict_format(monkeypatch):
    ...
    with patch("tekla_nest.providers.tekla_provider._find_bundled_python", return_value=None), \
         patch("tekla_nest.providers.tekla_provider._find_system_python", return_value="/usr/bin/python3"), \
...
def test_export_excel_summary_contains_project_and_milestone(tmp_path):
    ...
    summary = wb.active
    all_values = [
        str(cell.value or "") for row in summary.iter_rows() for cell in row
    ]
    ...
    assert any(
        "Project Name" in v or "Nome do projeto" in v for v in all_values
    ), "project_name label not in Summary sheet"
    assert any(
        "Milestone" in v or "Marco" in v for v in all_values
