...
try:
    from tekla_admin.api_client import (
        AdminApiClient,
        AdminClientError,
    ...
    # Running standalone from the monorepo checkout (not the installed workspace):
    # add the sibling package sources to the path.
    _PKG = Path(__file__).resolve().parents[2]
    for _p in (_PKG / "tekla-admin-console" / "src", _PKG / "tekla-common" / "src"):
        if str(_p) not in sys.path:
            sys.path.insert(0, str(_p))
    from tekla_admin.api_client import (  # noqa: E402
        AdminApiClient,
        AdminClientError,
        decode_json_response,
    )
    from tekla_admin.models import CreateLicenseRequest  # noqa: E402
