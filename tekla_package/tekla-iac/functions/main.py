...
def _bind_machine(data: dict, machine_id: str, doc_ref) -> https_fn.Response | None:
    ...
    if len(machines) >= max_machines:
        return _error(
            ...
            "Contact admin to release a slot.",
            409,
        )
    machines.append(machine_id)
    doc_ref.update(
        {
            "machine_id": existing or machine_id,
            "machines": machines,
            "activated_at": data.get("activated_at") or _now(),
            "last_validated": _now(),
        }
    )
    return None
...
@https_fn.on_request(**_runtime_options(max_instances=10, function_secrets=[JWT_PRIVATE_KEY]))
...
def admin_release(req: https_fn.Request) -> https_fn.Response:
    ...
    def handler(r):
        ...
        return _json_resp(
            {"license_key": key, "machines_remaining": len(update.get("machines", []))}
        )
