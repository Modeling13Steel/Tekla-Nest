...
def test_activate_saves_token_and_uses_normalized_server_url(license_env, monkeypatch):
...
def test_validate_allows_graceful_offline_revalidation(license_env, tmp_path, monkeypatch):
    reset_config()
    cfg = tmp_path / "config.yaml"
    cfg.write_text(
        "licensing:\n  required: true\n  server_url: http://license.test\n  revalidation_days: 0\n",
...
def test_validate_revoked_response_deactivates_local_license(license_env, tmp_path, monkeypatch):
    reset_config()
    cfg = tmp_path / "config.yaml"
    cfg.write_text(
        "licensing:\n  required: true\n  server_url: http://license.test\n  revalidation_days: 0\n",
