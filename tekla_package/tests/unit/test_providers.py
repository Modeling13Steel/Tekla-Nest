...
class TestTeklaPartProvider:
        ...
        with patch("tekla_nest.services.tekla_api.find_tekla_bin", return_value=r"C:\Tekla\bin"):
            with patch("tekla_nest.services.tekla_api.load_tekla_assemblies"):
                with patch("tekla_nest.services.tekla_api.connect_model"):
                    with patch(
                        "tekla_nest.services.tekla_api.get_selected_objects",
                        return_value=iter([bar, plate, chapa]),
                    ):
    ...
    def test_groups_by_reference(self):
        ...
        objects = [
            ...
            self._make_obj(
                {"PROFILE": "HEA240", "MATERIAL": "S275", "PART_POS": "B1", "LENGTH": 5000.0}
            ),
        ]
    ...
    def test_empty_selection(self):
        ...
        with patch("tekla_nest.services.tekla_api.find_tekla_bin", return_value=r"C:\Tekla\bin"):
            with patch("tekla_nest.services.tekla_api.load_tekla_assemblies"):
                with patch("tekla_nest.services.tekla_api.connect_model"):
                    with patch(
                        "tekla_nest.services.tekla_api.get_selected_objects", return_value=iter([])
                    ):
                        parts = TeklaPartProvider().get_parts()
    ...
    def test_filters_blank_parts(self):
        ...
        blank = self._make_obj({"PROFILE": "", "MATERIAL": "", "PART_POS": "", "LENGTH": 0.0})
        ...
        with patch("tekla_nest.services.tekla_api.find_tekla_bin", return_value=r"C:\Tekla\bin"):
            with patch("tekla_nest.services.tekla_api.load_tekla_assemblies"):
                with patch("tekla_nest.services.tekla_api.connect_model"):
                    with patch(
                        "tekla_nest.services.tekla_api.get_selected_objects",
                        return_value=iter([valid, blank, no_ref, zero_len]),
                    ):
                        parts = TeklaPartProvider().get_parts()
