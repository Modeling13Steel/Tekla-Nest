...
def test_summary_renders_all_fields(app) -> None:
    strip = KpiStrip()
    summary = OptimizationSummary(waste_pct=12.5, bars_used=3, unfit_count=0, profile_count=2)
...
def test_unfit_warning_property_set(app) -> None:
    strip = KpiStrip()
    summary = OptimizationSummary(waste_pct=5.0, bars_used=2, unfit_count=2, profile_count=1)
    strip.set_summary(summary)
