...
from tekla_admin.models import LicenseRecord
from tekla_admin.widgets.license_table import LicenseTableWidget
from tekla_admin.window import AdminWindow
...
def test_license_record_status_values():
    ...
    assert LicenseRecord("key", "Acme", machines=["m1"], max_machines=1).status(now) == "full"
    assert LicenseRecord("key", "Acme", machines=["m1"], max_machines=2).status(now) == "active"
