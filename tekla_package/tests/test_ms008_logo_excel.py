...
def test_excel_two_sheets_only(tmp_path):
    ...
    assert len(wb.sheetnames) == 2, f"Expected 2 sheets, got {len(wb.sheetnames)}: {wb.sheetnames}"
...
def test_excel_summary_has_profile_data(tmp_path):
    ...
    assert any(profile_name in v for v in all_values), (
        f"Profile name {profile_name!r} not found in Summary sheet. Values found: {all_values[:20]}"
    )
...
def test_pillow_in_dependencies():
    ...
    toml_path = Path(__file__).parent.parent / "tekla-nest" / "pyproject.toml"
    content = toml_path.read_text(encoding="utf-8")
    assert "Pillow" in content, (
        "Pillow not found in pyproject.toml. Add 'Pillow>=9.0' to the dependencies array."
    )
