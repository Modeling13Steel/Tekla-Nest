...
def modal_spy(monkeypatch: pytest.MonkeyPatch) -> list[tuple[str, str, str]]:
    ...
    monkeypatch.setattr(QMessageBox, "warning", _make("warning", QMessageBox.StandardButton.Ok))
    monkeypatch.setattr(
        QMessageBox, "information", _make("information", QMessageBox.StandardButton.Ok)
    )
    monkeypatch.setattr(QMessageBox, "critical", _make("critical", QMessageBox.StandardButton.Ok))
    monkeypatch.setattr(QMessageBox, "question", _make("question", QMessageBox.StandardButton.Yes))
    return calls
...
class SignalCollector:
    ...
    def __init__(self, presenter: NestPresenter) -> None:
        ...
        presenter.operation_failed.connect(lambda op, msg: self.failed_ops.append((op, msg)))
    ...
    class _Journey:
        ...
        def calculate_async(self, qtbot, timeout_ms: int = 3000) -> None:
            with qtbot.waitSignal(
                self.presenter.operation_completed,
                timeout=timeout_ms,
