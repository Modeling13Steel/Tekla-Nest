...
def test_auto_stock_seeds_from_parts_edited_in_table(window, presenter):
    ...
    window._parts_table.set_parts(
        [
            ...
            PartEntry(quantity=1, length=4500, reference="P2", profile="IPE200", material="S275JR"),
        ]
    )
...
def test_calculate_picks_up_stock_edited_in_table(window, presenter):
    ...
    window._parts_table.set_parts(
        [
            PartEntry(quantity=1, length=3000, reference="P1", profile="IPE200", material="S275JR"),
        ]
    )
    ...
    assert any(s.profile == "IPE200" and s.source == "Cliente" for s in presenter._client_stock)
    presenter.run_optimization()
    result = presenter._last_result
