...
class EditableTableModel[T](QAbstractTableModel):
    ...
    def remove_rows(self, rows: Iterable[int]) -> int:
        ...
        """
        valid = sorted({r for r in rows if 0 <= r < len(self._data)}, reverse=True)
        if not valid:
            return 0
        self.beginResetModel()
        for r in valid:
...
class DataTableWidget[T](QWidget):
    ...
    def __init__(
        ...
        if self._material_column is not None:
            self._table.setItemDelegateForColumn(
                self._material_column,
                MaterialChipDelegate(self._table),
            )
        set_accessibility(
            self._table,
            self._translated_accessible_name(),
    ...
    def delete_selected_rows(self) -> int:
        ...
        if not proxy_rows:
            return 0
        source_rows = [self._proxy_model.mapToSource(idx).row() for idx in proxy_rows]
