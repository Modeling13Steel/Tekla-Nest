...
def load_stock_csv(path: Path | str) -> list[StockEntry]:
    ...
    path = Path(path)
    _log(
        f"load_stock_csv START  path={path}  exists={path.exists()}  size={path.stat().st_size if path.exists() else 'N/A'}"
    )
    _check_file(path)
    ...
    _log(
        f"load_stock_csv text read  len={len(text)}  first_line={text.splitlines()[0][:120] if text else '<empty>'!r}"
    )
...
def load_parts_csv(path: Path | str) -> list[PartEntry]:
    ...
    path = Path(path)
    _log(
        f"load_parts_csv START  path={path}  exists={path.exists()}  size={path.stat().st_size if path.exists() else 'N/A'}"
    )
    _check_file(path)
    ...
    text = _read_text(path)
    _log(
        f"load_parts_csv text read  len={len(text)}  first_line={text.splitlines()[0][:120] if text else '<empty>'!r}"
    )
    sep = _detect_separator(text, get_config().csv_separators)
