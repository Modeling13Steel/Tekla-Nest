...
def export_excel(
    ...
    try:
        ...
        if logo_path.exists() and logo_path.suffix.lower() in (
            ".png",
            ".jpg",
            ".jpeg",
            ".bmp",
        ...
        LOGGER.warning(
            "Pillow not installed; Excel logo will be skipped. Run: pip install Pillow>=9.0"
        )
    ...
    ws_summary.cell(row=current_row, column=2, value=datetime.now().strftime("%Y-%m-%d %H:%M"))
    current_row += 1
    ...
    prep_by_key = {
        (p.profile.lower().strip(), (p.material or "").lower().strip()): p for p in preps
    }
    ...
    for prof in result.profiles:
        ...
        for bar_idx, bar in enumerate(prof.bars):
            ...
            ws_summary.cell(
                row=current_row, column=4, value=", ".join(str(int(c)) for c in bar.cuts)
            )
...
def _write_purchase_sheet(wb, result, cfg, labels, header_font, header_fill, wrap) -> None:
    ...
    for prof in result.profiles:
        for bar in prof.bars:
            key = (prof.profile, prof.material or bar.material, float(bar.original_length))
