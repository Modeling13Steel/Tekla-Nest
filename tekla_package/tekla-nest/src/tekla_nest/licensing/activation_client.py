...
async def activate(
    ...
    if response.status_code != 200:
        return Err(f"Activation server returned HTTP {response.status_code}: {response.text[:500]}")
    ...
    if not verify_signature(lic.signable_bytes(), lic.signature, get_public_key_bytes()):
